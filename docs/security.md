...
The inbound secret and enabled local-member secrets resolve and must be
header-safe visible ASCII during service construction. Provider credentials
remain lazy: building the registry, listing
...
Credential commands accept no more than 32 arguments, 4 KiB per argument, or
16 KiB total, and reject empty arguments and control characters. They run with
a cleared environment plus `PATH`, null stdin and stderr, piped stdout,
process-kill-on-drop, a five-second timeout, and a 4 KiB output limit. Output
must be non-empty visible ASCII without whitespace.
...
Inbound headers, bodies, virtual model identifiers, rate, and concurrency are
bounded before routing. Configured physical model identifiers, credential
argv, upstream deadlines, and member/provider semaphore sizes are also bounded.
...
The supplied systemd profile isolates ChatGPT-managed CLI state under
`/var/lib/octoroute/codex` through `CODEX_HOME`; `ProtectHome=true` prevents an
accidental dependency on other home-directory state.
