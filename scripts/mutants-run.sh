...
# Octoroute's gateway tests spawn fixtures such as `endless-codex`, a loop that
# never exits on its own. cargo-mutants kills its own `cargo test` children, but
# the mutants that disable the process-group kill path are exactly the ones that
# time out, so their fixtures outlive the run and each holds a core. Two were
# once found holding 15 CPU-minutes. Kill every process whose command line names
# this run's TMPDIR, excluding this script's own ancestry, which a caller could
# have started with TMPDIR in its arguments.
self_and_ancestors() {
  local pid=$$
  while [ "$pid" -gt 1 ]; do
    printf '%s ' "$pid"
...
trap 'reap_mutation_orphans; cleanup_mutation_scratch' EXIT
...
# The host lock is descriptor 9, held here or by the remote session. A fixture
# that outlives its mutant must not inherit it and wedge the next sweep.
cargo mutants -j "${MUTANTS_JOBS:-4}" --no-shuffle --minimum-test-timeout 120 \
  --cap-lints true --output "$OUT_DIR" "$@" 9<&- && status=0 || status=$?
