...
# is still using. (Measured in drep, where this script comes from, on 12 mutants from src/docs/fence.rs: a local M5 Max at -j 4 took 1m54 with the machine pinned.) The offload runs as this repository's octoroute-mutants role on ai-1 and costs this machine nothing.
#
...
#                        (default: .cache/octoroute-mutants/<repo name>)
#   DREP_MUTANTS_REMOTE_HOST_LOCK
#                        absolute lock path shared with the hosted runner
#                        (default: /srv/ci/fleet/octoroute-mutants/home/host.lock)
...
REMOTE_DIR="${DREP_MUTANTS_DIR:-.cache/octoroute-mutants/$(basename "$PWD")}"
REMOTE="$HOST:$REMOTE_DIR"
JOBS="${MUTANTS_JOBS:-4}"
REMOTE_HOST_LOCK="${DREP_MUTANTS_REMOTE_HOST_LOCK:-/srv/ci/fleet/octoroute-mutants/home/host.lock}"
