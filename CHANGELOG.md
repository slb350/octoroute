...
- The mutation scripts are now drep's five, copied with this repository's role. Hosted and local runs share the role's host lock, a second run in one checkout waits for the first instead of deleting its results and scratch, and each checkout mirrors into its own directory on ai-1. A commit with no staged Rust changes exits before the lock. The staged hook mutation-tests what is being committed: it writes the index to a tree, builds a copy of that tree on ai-1 or locally, and fails if the index changed during the run, because `git commit` reads the index after the hook. Unstaged edits and untracked files no longer reach the run. The hook still refuses a working tree that differs from the index before fmt and clippy, which read it; that check now lives in the hook rather than the shared scripts. Their tests are drep's contract and lock tests plus behavioural tests of cleanup, the verdict, the snapshot and the hook.
