...
pub struct ModelsConfig {
    pub fast: ModelEndpoint,
    pub balanced: ModelEndpoint,
    pub deep: ModelEndpoint,
}
...
"#;
    ...
    fn test_config_parses_model_endpoints() {
        let config = Config::from_str(TEST_CONFIG).expect("should parse config");
        assert_eq!(config.models.fast.name, "qwen/qwen3-vl-8b");
        assert_eq!(config.models.fast.base_url, "http://192.168.1.67:1234/v1");
        assert_eq!(config.models.fast.max_tokens, 4096);
        assert_eq!(config.models.balanced.name, "qwen/qwen3-30b-a3b-2507");
        assert_eq!(config.models.deep.max_tokens, 16384);
    }
